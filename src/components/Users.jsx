        ...
        const usersList = allUsers.map((user, index) => {
            ...
            return user.isEditing === true ? (
                ...
                <tr  key={index}>
                    <td>{user.name}</td>
                    <td>{user.age}</td>
                    <td><button className="btn white black-text" onClick={() => pressEditBtn(index)}>Editar</button>  |  <button className="btn red" onClick={()=>pressDelete(index)}>Eliminar</button></td>
                </tr>
